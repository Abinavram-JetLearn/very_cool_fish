...
def explosionbomb():
    ...
    for explosion in explosions:
        ...
        if explosion.image == "explosion2":
            explosion.image = "explosion3"

        if explosion.image == "explosion3":
            explosion.image = "explosion4"

        if explosion.image == "explosion4":
            explosion.image = "explosion5"

        if explosion.image == "explosion5":
            explosion.image = "explosion6"

        if explosion.image == "explosion6":
            explosions.remove(explosion)
    clock.schedule(explosionbomb, 0.1)
clock.schedule(explosionbomb, 0.1)

def draw():
    global timer, explosions
    ...
    if game == True:
        ...
        for enemy in enemies:
            ...
            if enemy.image == "greenenemy":
                ...
                animate(enemy, pos = player.pos, angle = enemy.angle_to(player.pos) - 90, duration = 0.5,)
